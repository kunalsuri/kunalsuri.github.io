...
- **"What Is …" authoring pipeline**: `docs/series/what-is-playbook.md` (single source of truth for house style and the verification gate), `docs/series/what-is-backlog.md` (idea capture), per-post verification reports in `docs/series/reviews/`, and a single `what-is` Agent Skill in `.claude/skills/` (mirrored to `.agents/skills/`). `/what-is <topic-or-slug>` derives the stage from the disk — no post yet, drafted, verified, or published — and does the next thing, so pipeline state is never tracked in a second place that could drift. It sets `disable-model-invocation: true`, so Claude cannot draft or publish on its own initiative; the human review turn is enforced by the harness rather than by instructions.
...
### Changed
- The series pipeline is one command rather than four. `/what-is` reads the posts on disk to decide what to do next, `scripts/verify-post.mjs --stage` reports the same, and `docs/series/what-is-backlog.md` went back to being ideas only — the status tables it used to carry duplicated what the front-matter already said.
- Dropped the 700-word minimum for What Is posts. A term explained clearly in 300 words is the series working, so the floor was fighting the goal; only a soft ~1200-word ceiling warns now.
